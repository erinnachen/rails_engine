...
RSpec.describe "GET /api/v1/merchants/most_items?quantity=" do
  include SpecHelpers
  xit "returns top merchants ranked by total items sold" do
    ...
    invoice2.invoice_items << create(:invoice_item, quantity: 4, unit_price: 5635)
    invoice3.invoice_items << create(:invoice_item, quantity: 1, unit_price: 550)

    get "/api/v1/merchants/most_items?quantity=1"

    expect(response.status).to eq 200
    merchants = parsed_body
